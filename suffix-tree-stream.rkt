...
(define (get-ch-branch st ch)
  (cond
    ((st-empty? st) #f) ; check if the ST is empty (there are no more branches to be checked)
    ((equal? (car (get-branch-label (first-branch st))) ch) (first-branch st)) ; if the first letter of the label from the first branch is equal to ch, return it
    (else (get-ch-branch (other-branches st) ch)))) ; if the label from the first branch is not equal to ch, continue recursively by checking the other branches
