...
(define-syntax-rule (collection-empty? col)
  (if (stream-empty? col)
      #t
      #f))

(define-syntax-rule (collection-first col)
  (stream-first col))

(define-syntax-rule (collection-rest col)
  (stream-rest col))

(define-syntax-rule (collection-second col)
  (collection-first (collection-rest col)))

(define-syntax-rule (collection-filter f col)
  (stream-filter f col))

(define-syntax-rule (collection-map f col)
  (stream-map f col))

(define-syntax-rule (collection-foldl f acc col)
  (stream-fold f acc col))
